...
$mail->addAddress($_POST['email'], 'Khách Hàng');
...
$mail->Body    = "Kính chào:".$_POST['firstname']." ".$_POST['lastname']."! \nQuý vị đã đăng ký thành viên tại ZuraVn. \nXin vui lòng nhấn vào đây để kích hoặc tài khoản !\n http://localhost/project/izcms/admin/activate.php?x=".urlencode($email)."&y={$a}";
...
if (!$mail->send()) {
    ...
    echo'<script>alert("Yêu cầu của quý khách đã được thực hiện, check email: '.$_POST['email'].' để kích hoạt tài khoản. !")</script>';
